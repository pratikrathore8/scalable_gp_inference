...
filtered = filter_runs(
    ...
    require_all={
        ...
        CONFIG_KEYS["SOLVER"]: ["sap", "pcg", "sdd"]
    }
)
...
runs_data = organize_runs_data(
    selected_runs,
    [METRIC_PATHS["TEST_RMSE"], METRIC_PATHS["POSTERIOR_MEAN_NLL"]],
    "datapasses"
)
...
plotter.plot_single_metric(
    ...
    x_axis=X_AXIS_OPTIONS["DATAPASSES"],
    save_path=Path(BASE_SAVE_DIR) / DATASET / f"TEST_RMSE_{TIMESTAMP}")

# Metric grid plot
plotter.plot_metric_grid(
    y_metrics=[METRIC_PATHS["TEST_RMSE"], METRIC_PATHS["POSTERIOR_MEAN_NLL"]],
    x_axis=X_AXIS_OPTIONS["DATAPASSES"],
    save_path=Path(BASE_SAVE_DIR) / DATASET / f"TEST_RMSE_POSTERIOR_MEAN_NLL_{TIMESTAMP}")